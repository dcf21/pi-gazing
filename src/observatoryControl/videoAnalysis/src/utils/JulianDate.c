...
void SwitchOverCalDate(double *LastJulian, double *FirstGregorian)
 {
  *LastJulian = 17520902.0; *FirstGregorian = 17520914.0; // British
  return;
 }
...
double JulianDay(int year, int month, int day, int hour, int min, int sec, int *status, char *errtext)
 {
  ...
  if (ReqDate <= LastJulian)
   ...
   { *status=1; sprintf(errtext, "The requested date never happened in the British calendar: it was lost in the transition from the Julian to the Gregorian calendar."); return 0.0; }
