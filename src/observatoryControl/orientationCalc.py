...
def orientationCalc(cameraId, utcNow, utcMustStop=0):
    ...
    for f in acceptableFiles:
        ...
        os.system("%s %s_tmp.png %s %s_tmp2.png" % (
        barrelCorrect, imgname, os.path.join(STACKER_PATH, "lenses", lensName), imgname))
        ...
        os.system(
            ...
            imgname, fractionX * d[0], fractionY * d[1], (1 - fractionX) * d[0] / 2, (1 - fractionY) * d[1] / 2,
            imgname))
    ...
    scalexList = [i['sx'] * deg for i in fits if i['fit']];
    scalexBest = mod_astro.meanAngle(scalexList)[0]
    scaleyList = [i['sy'] * deg for i in fits if i['fit']];
    scaleyBest = mod_astro.meanAngle(scaleyList)[0]
