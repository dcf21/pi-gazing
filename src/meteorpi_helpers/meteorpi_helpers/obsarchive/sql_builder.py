...
def search_observations_sql_builder(search):
    """
    Create and populate an instance of :class:`obsarchive_db.SQLBuilder` for a given
    :class:`obsarchive_model.ObservationSearch`. This can then be used to retrieve the results of the search, materialise
    them into :class:`obsarchive_model.Observation` instances etc.

    :param ObservationSearch search:
        The search to realise
    :return:
        A :class:`obsarchive_db.SQLBuilder` configured from the supplied search
    """
...
def search_obsgroups_sql_builder(search):
    """
    Create and populate an instance of :class:`obsarchive_db.SQLBuilder` for a given
    :class:`obsarchive_model.ObservationGroupSearch`. This can then be used to retrieve the results of the search,
    materialise them into :class:`obsarchive_model.ObservationGroup` instances etc.
...
WHERE x1.groupId=g.uid AND x3.publicId=%s)""")
    ...
    b.add_sql(search.child_observation_id, """
EXISTS (SELECT 1 FROM archive_obs_group_members y1
INNER JOIN archive_observations y2 ON y2.uid=y1.childObservation
WHERE y1.groupId=g.uid AND y2.publicId=%s)""")
    b.add_sql(search.child_group_id, """
EXISTS (SELECT 1 FROM archive_obs_group_members y1
INNER JOIN archive_obs_groups y2 ON y2.uid=y1.childGroup
WHERE y1.groupId=g.uid AND y2.publicId=%s)""")
...
def search_files_sql_builder(search):
    """
    Create and populate an instance of :class:`obsarchive_db.SQLBuilder` for a given
    :class:`obsarchive_model.FileRecordSearch`. This can then be used to retrieve the results of the search, materialise
    them into :class:`obsarchive_model.FileRecord` instances etc.
...
def search_metadata_sql_builder(search):
    """
    Create and populate an instance of :class:`obsarchive_db.SQLBuilder` for a given
    :class:`obsarchive_model.ObservatoryMetadataSearch`. This can then be used to retrieve the results of the search,
    materialise them into :class:`obsarchive_model.ObservatoryMetadata` instances etc.
