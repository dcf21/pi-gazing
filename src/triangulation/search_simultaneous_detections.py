...
from pigazing_helpers import dcf_ast, gnomonic_project, sunset_times
...
def search_simultaneous_detections(utc_min, utc_max, utc_must_stop):
    ...
    logging.info("{:6d} moving objects seen within this time period".
                 format(len(triggers_raw['obs'])))
    logging.info("{:6d} moving objects rejected because tagged as junk".
                 format(len(triggers_raw['obs']) - len(triggers)))
    logging.info("{:6d} simultaneous detections found.".
                 format(len(groups)))

    for item in groups:
        logging.info("{} -- {:3d} stations (time spread {:.1f} sec)".format(dcf_ast.date_string(item['time']),
    ...
    for item in groups:
        ...
        group = db.register_obsgroup(title="Multi-station detection", user_id="system", semantic_type=semantic_type,
                                     ...
                                     obs=item['ids'])
        logging.info("Simultaneous detection at {} by {:3d} stations (time spread {:.1f} sec)".
        ...
        for trigger in item['triggers']:
            if 'sight_line_list' in trigger:
                ...
                logging.info("Triangulated details of observation <{}> is {}.".
                             format(trigger['obs'].id, triangulation_info))
...
def flush_simultaneous_detections(utc_min, utc_max, utc_must_stop):
    db = obsarchive_db.ObservationDatabase(file_store_path=settings['dbFilestore'],
                                           db_host=installation_info['mysqlHost'],
                                           db_user=installation_info['mysqlUser'],
                                           db_password=installation_info['mysqlPassword'],
