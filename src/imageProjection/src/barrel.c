...
Usage: barrel.bin <filename> <barrel_a> <barrel_b> <barrel_c> <output filename>\n\
    ...
    if (argv[i][0]!='-')
     {
      if (HaveFilename > 4)
       {
        sprintf(temp_err_string, "barrel.bin should be provided with three barrel-distortion coefficients, and two filenames on the command line. Too many filenames appear to have been supplied. Type 'barrel.bin -help' for a list of available commandline options.");
  ...
  if (HaveFilename < 5)
   {
    sprintf(temp_err_string, "barrel.bin should be provided with three barrel-distortion coefficients, and two filenames on the command line. Type 'barrel.bin -help' for a list of available commandline options.");
    gnom_error(ERR_GENERAL, temp_err_string);
    return 1;
   }
