...
image_ptr image_get(char *filename)
 {
  ...
  if (png_ptr == NULL) { gnom_error(ERR_MEMORY, "Out of memory"); goto finalise; }

  info_ptr = png_create_info_struct(png_ptr);
  if (info_ptr == NULL) { gnom_error(ERR_MEMORY, "Out of memory"); goto finalise; }

  if (setjmp(png_jmpbuf(png_ptr))) { gnom_error(ERR_INTERNAL, "Unexpected error in libpng while trying to decode PNG image file"); goto finalise; }
  ...
  if (DEBUG) { sprintf(temp_err_string, "Size %dx%d", width, height); gnom_log(temp_err_string); }
  if (DEBUG) { sprintf(temp_err_string, "Depth %d", depth); gnom_log(temp_err_string); }
  ...
  if ( (png_colour_type == PNG_COLOR_TYPE_GRAY) || (png_colour_type == PNG_COLOR_TYPE_GRAY_ALPHA)) colour = BMP_COLOUR_GREY;
  if ( (png_colour_type == PNG_COLOR_TYPE_RGB ) || (png_colour_type == PNG_COLOR_TYPE_RGB_ALPHA )) colour = BMP_COLOUR_RGB;

  if (png_colour_type == PNG_COLOR_TYPE_PALETTE)
   {
    colour = BMP_COLOUR_PALETTE;
    i = png_get_PLTE(png_ptr,info_ptr,&pngpalette,&ncols);
    if (i==0) { gnom_error(ERR_FILE, "PNG image file claims to be paletted, but no palette was found"); goto finalise; }
    pal_len = ncols;
    palette = malloc(ncols*3);
    if (palette == NULL) { gnom_error(ERR_MEMORY, "Out of memory"); goto finalise; }
    for (i=0; i<ncols; i++)
     {
    ...
    if (DEBUG) { sprintf(temp_err_string, "PNG image file contains a palette of %d colours", ncols); gnom_log(temp_err_string); }
   }
  ...
  if (data == NULL) { gnom_error(ERR_MEMORY, "Out of memory"); goto finalise; }

  // libpng requires a separate pointer to each row of image
  row_ptrs = (png_bytep *)malloc(height*sizeof(png_bytep));
  if (row_ptrs == NULL) { gnom_error(ERR_MEMORY, "Out of memory"); data = NULL; goto finalise; }
  ...
  if (png_get_valid(png_ptr,info_ptr,PNG_INFO_tRNS))
   {
    ...
    if (DEBUG) { sprintf(temp_err_string, "PNG has %d transparent entries in palette", ntrans); gnom_log(temp_err_string); }
    if (png_colour_type == PNG_COLOR_TYPE_PALETTE)
     {
      ...
      if (j!=1) { gnom_warning(ERR_FILE, "PNG has transparency, but not in the form of a single fully colour in its palette. Such transparency is not supported."); }
      else
       {
