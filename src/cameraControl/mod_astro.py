...
def rs_riseculmgap(decObj, latObs, angBelowHorizon):  # all inputs are in radians
    ...
    B = atan2(cosO * cos(angBelowHorizon),
              (sinO * cos(angBelowHorizon) * cos(alpha) - sin(angBelowHorizon) * sin(alpha)))
    if isnan(B):
...
def meanAngle2D(posList):
    ...
    sd = sqrt(sum(
        [(xlist[i] - xmean) ** 2 + (ylist[i] - ymean) ** 2 + (zlist[i] - zmean) ** 2 for i in range(len(xlist))]) / len(
        posList))
    asd = atan(sd)  # Find angular spread of points as seen from centre
