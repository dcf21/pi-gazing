                ...
                if f.endswith(".%s" % inExt):
                    ...
                    for outDir in outDirs:
                        if outDir in fileCensus:
                            if (
                                inExt != 'h264'):  # Most files produce output with the same timestamp as the input; except overnight videos...
                                if (utc in fileCensus[outDir]): done = True
                            else:
                                for u in fileCensus[outDir].keys():
                                    if ((u > utc) and (
                                        u < utc + VIDEO_MAXRECTIME)):  # For files in rawvideo, assume they are processed if we have an output file within some duration of the video's start time
                                        done = True
                                        break
...
for rowHead in rows:
    ...
    for colHead in cols:
        if rowHead in jobCensus[colHead]:
            out.write("%25s " % jobCensus[colHead][rowHead])
        else:
            out.write("%25s " % "-----")
    out.write("\n")
