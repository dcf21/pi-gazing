...
int read_config(const char *filename, settings *feed_s, settings_input *si, settings_input *s_in_default, int *image_count) {
    ...
    while (!feof(in_file)) {
        ...
        if (strcmp(key, "GNOMONIC") == 0) {
            ...
            feed_s->exposure_compensation = getFloat(cp, NULL);
            ...
            feed_s->x_size = getFloat(cp, NULL);
            ...
            feed_s->y_size = getFloat(cp, NULL);
            ...
            feed_s->x_scale = getFloat(cp, NULL) * M_PI / 180.;
            ...
            feed_s->y_scale = getFloat(cp, NULL) * M_PI / 180.;
            ...
            feed_s->linear_rotation = getFloat(cp, NULL);
            ...
            if (strcmp(key2, "output") == 0) {
                char *cp = key_value;
                while (!isalnum(*cp) && (*cp != '/') && (*cp != '\0')) cp++;
                getWord(feed_s->output_filename, cp, FNAME_LENGTH);
                ...
                s_in_default->background_subtract = getFloat(cp, NULL);
                ...
                feed_s->cloud_mask = getFloat(cp, NULL);
        ...
        } else if (strcmp(key, "ADD") == 0) // This is a source image to be stacked
        {
            ...
            *feed_si = *s_in_default;
            logging_report("\nNew Image:");
            getWord(feed_si->input_filename, cp, FNAME_LENGTH);
            ...
            feed_si->weight_in = getFloat(cp, NULL);
            sprintf(temp_err_string, "Image weight = %.6f", feed_si->weight_in);
            logging_report(temp_err_string);
            cp = nextWord(cp);
            if (!validFloat(cp, NULL)) logging_fatal(__FILE__, __LINE__, "Could not read exposure compensation");
            feed_si->exposure_compensation_in = getFloat(cp, NULL);
            ...
            feed_si->x_size_in = getFloat(cp, NULL);
            ...
            feed_si->y_size_in = getFloat(cp, NULL);
            ...
            if (feed_s->mode == MODE_GNOMONIC) {
                ...
                if (!validFloat(cp, NULL)) logging_fatal(__FILE__, __LINE__, "Could not read central RA");
                feed_si->ra0_in = getFloat(cp, NULL) * M_PI / 12.;
                sprintf(temp_err_string, "Central RA = %.6f hr", feed_si->ra0_in / M_PI * 12.);
                logging_report(temp_err_string);
                cp = nextWord(cp);
                if (!validFloat(cp, NULL)) logging_fatal(__FILE__, __LINE__, "Could not read central Dec");
                feed_si->dec0_in = getFloat(cp, NULL) * M_PI / 180.;
                ...
                feed_si->rotation_in = getFloat(cp, NULL) * M_PI / 180.;
                ...
                feed_si->x_scale_in = getFloat(cp, NULL) * M_PI / 180.;
                ...
                feed_si->y_scale_in = getFloat(cp, NULL) * M_PI / 180.;
                ...
                if (!validFloat(cp, NULL)) logging_fatal(__FILE__, __LINE__, "Could not read x offset");
                feed_si->x_off_in = getFloat(cp, NULL);
                sprintf(temp_err_string, "X Shift = %.2f pixels", feed_si->y_off_in);
                logging_report(temp_err_string);
                cp = nextWord(cp);
                if (!validFloat(cp, NULL)) logging_fatal(__FILE__, __LINE__, "Could not read y offset");
                feed_si->y_off_in = getFloat(cp, NULL);
                sprintf(temp_err_string, "Y Shift = %.2f pixels", feed_si->x_off_in);
                logging_report(temp_err_string);
                cp = nextWord(cp);
                if (!validFloat(cp, NULL)) logging_fatal(__FILE__, __LINE__, "Could not read linear rotation");
                feed_si->linear_rotation_in = getFloat(cp, NULL) * M_PI / 180.;
