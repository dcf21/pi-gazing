...
class ObservationDatabaseImportReceiver(object):
    """
    Connects to a
    :class:`obsarchive_db.ObservationDatabase` and pushes any data to it on import, including managing the acquisition of any
...
class ImportRequest(object):
    ...
    def response_need_file_data(file_id):
        ...
        :param string file_id:
            the UUID of the :class:`obsarchive_model.FileRecord` for which we don't currently have data
        :return:
            A response that can be returned from a Flask service method
        """
    ...
    def _get_entity(entity_id):
        """
        Uses the request context to retrieve a :class:`obsarchive_model.CameraStatus`, :class:`obsarchive_model.Event` or
        :class:`obsarchive_model.FileRecord` from the POSTed JSON string.
...
def add_routes(obsarchive_app, url_path='/importv2'):
    """
    Add two routes to the specified instance of :class:`obsarchive_server.ObservationApp` to implement the import API and allow
    for replication of data to this server.

    :param obsarchive_server.ObservationApp obsarchive_app:
        The :class:`obsarchive_server.ObservationApp` to which import routes should be added
    :param obsarchive_server.importer_api.BaseImportReceiver handler:
        A subclass of :class:`obsarchive_server.importer_api.BaseImportReceiver` which is used to handle the import. If
        not specified this defaults to an instance of :class:`obsarchive_server.importer_api.ObservationDatabaseImportReceiver`
        which will replicate any missing information from the import into the database attached to the obsarchive_app.
    ...
    @obsarchive_app.requires_auth(roles=['import'])
    def import_entities():
        """
        Receive an entity import request, using :class:`obsarchive_server.import_api.ImportRequest` to parse it, then
        passing the parsed request on to an instance of :class:`obsarchive_server.import_api.BaseImportReceiver` to deal
        ...
        handler = ObservationDatabaseImportReceiver(db=db)
    ...
    def import_file_data(file_id_hex, md5_hex):
        ...
        :param string file_id_hex:
            The hex representation of the :class:`obsarchive_model.FileRecord` to which this data belongs.
        """
        db = obsarchive_app.get_db()
        handler = ObservationDatabaseImportReceiver(db=db)
        file_id = file_id_hex
