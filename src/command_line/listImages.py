...
INNER JOIN archive_semanticTypes ast on archive_files.semanticType = ast.uid
WHERE """ + " AND ".join(where) + """;
""", args)
        ...
        for count, item in enumerate(files):
            if count > 0:
                sys.stdout.write("\n{:91s}".format(""))
            sys.stdout.write("{:27s} --> {:32s}".format(item['fileName'], item['repositoryFname'], ""))
        sys.stdout.write("\n")
