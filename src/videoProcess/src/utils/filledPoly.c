...
void fillPolygonsFromFile(FILE *infile, unsigned char *mask, int width, int height)
 {
  ...
  while (!stopping)
   {
    if (feof(infile)) { stopping=1; goto blankLine; }
...
blankLine:
  ...
  if (filledPixels<1) memset(mask, 1, width*height); // If not clipping region is specified, allow triggers across the whole frame
  return;
 }
  ...
  for (pixelY=0; pixelY<height; pixelY++)
   {
    ...
    while (i<nodes-1)
     {
      if (nodeX[i]>nodeX[i+1]) { swap=nodeX[i]; nodeX[i]=nodeX[i+1]; nodeX[i+1]=swap; if (i) i--; }
      else { i++; }
     }
      ...
      if   (nodeX[i+1]> 0 )
       {
        ...
        for (pixelX=nodeX[i]; pixelX<nodeX[i+1]; pixelX++) { mask[ pixelX + width*pixelY ]=1; Nfilled++; }
       }
     }
   }
  return Nfilled;
