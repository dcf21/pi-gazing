...
static inline int testPixel(observeStatus *os, const int *image1, const int *image2, const int o, const int threshold)
 {
  ...
  if (image1[o]-image2[o]>threshold) // Search for pixels which have brightened by more than threshold since past image
   {
    ...
    for (x=margin;x<os->width-margin; x++)
     {
      ...
      triggerR[o] = CLIP256( (image1[o]-image2[o])*64/thresholdTrigger ); // RED channel - difference between images B and A
      triggerG[o] = CLIP256( os->pastTriggerMap[o] * 256 / (2.3*pastTriggerMapAverage) ); // GRN channel - map of pixels which are excluded for triggering too often
      triggerB[o] = 0;
      if ((os->mask[o]) && testPixel(os,image1,image2,o,thresholdMonitor))
       {
        os->pastTriggerMap[o]++;
        if (testPixel(os,image1,image2,o,thresholdTrigger)) // Search for pixels which have brightened by more than threshold since past image
         {
          os->pastTriggerMap[o]++;
            ...
            if (blockId==0)
             {
              ...
              os->triggerBlock_top[blockId]=y; os->triggerBlock_bot[blockId]=y;
