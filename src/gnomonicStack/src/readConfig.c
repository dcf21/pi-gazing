// readConfig.c
// $Id: readConfig.c 1137 2014-11-24 22:18:14Z pyxplot $
...
int readConfig(char *filename, settings *feed_s, settingsIn *si, settingsIn *s_in_default, int *nImages)
 {
  ...
  if ((infile = fopen(filename,"r"))==NULL) { sprintf(temp_err_string, "Stacker could not open input file '%s'.", filename); gnom_error(ERR_GENERAL, temp_err_string); return 1; }
  file_linenumber = 0;
  while (!feof(infile))
   {
    file_readline(infile, line, LSTR_LENGTH); file_linenumber++;
    StrStrip(line,line);
    ...
    if (strcmp(key, "GNOMONIC")==0)
     {
      ...
      sprintf(temp_err_string, "XScale = %.6f deg/width", feed_s->XScale/M_PI*180.); gnom_report(temp_err_string);
      cp=NextWord(cp);
      ...
      sprintf(temp_err_string, "YScale = %.6f deg/height", feed_s->YScale/M_PI*180.); gnom_report(temp_err_string);
     }
    else if (strcmp(key, "FLAT")==0)
     {
      ...
      sprintf(temp_err_string, "XOff = %6d pixel", feed_s->XOff); gnom_report(temp_err_string);
      cp=NextWord(cp);
      if (!ValidFloat(cp,NULL)) gnom_fatal(__FILE__,__LINE__,"Could not read y offset");
      feed_s->YOff = GetFloat(cp,NULL);
      sprintf(temp_err_string, "Yoff = %6d pixel", feed_s->YOff); gnom_report(temp_err_string);
      cp=NextWord(cp);
      if (!ValidFloat(cp,NULL)) gnom_fatal(__FILE__,__LINE__,"Could not read linear rotation");
      feed_s->LinearRot = GetFloat(cp,NULL);
      sprintf(temp_err_string, "LinearRot = %.6f deg", feed_s->LinearRot); gnom_report(temp_err_string);
     }
    else if (strcmp(key, "SET")==0)
     {
      ...
      key2[i++]='\0'; 
      sprintf(temp_err_string, "SET %s", key2); gnom_report(temp_err_string);
      ...
      if (strcmp(key2, "output")==0)
      ...
      else if (strcmp(key2, "backgroundsub")==0)
       {
        ...
        sprintf(temp_err_string, "backgroundsub = %d", s_in_default->backSub); gnom_report(temp_err_string);
       }
      else if (strcmp(key2, "cloudmask")==0)
       {
        ...
        sprintf(temp_err_string, "cloudmask = %d", feed_s->cloudMask); gnom_report(temp_err_string);
       }
      continue;
     }
      ...
      *feed_si = *s_in_default;
      ...
      sprintf(temp_err_string, "Input filename = %s", feed_si->InFName); gnom_report(temp_err_string);
      cp=NextWord(cp);
      if (!ValidFloat(cp,NULL)) gnom_fatal(__FILE__,__LINE__,"Could not read image weight");
      feed_si->InWeight = GetFloat(cp,NULL);
      sprintf(temp_err_string, "Image weight = %.6f", feed_si->InWeight); gnom_report(temp_err_string);
      cp=NextWord(cp);
      if (!ValidFloat(cp,NULL)) gnom_fatal(__FILE__,__LINE__,"Could not read exposure compensation");
      feed_si->InExpComp = GetFloat(cp,NULL);
      sprintf(temp_err_string, "Exposure compensation = %.6f", feed_si->InExpComp); gnom_report(temp_err_string);
      cp=NextWord(cp);
      if (!ValidFloat(cp,NULL)) gnom_fatal(__FILE__,__LINE__,"Could not read x size");
      feed_si->InXSize = GetFloat(cp,NULL);
      sprintf(temp_err_string, "XSize = %6d", feed_si->InXSize); gnom_report(temp_err_string);
      cp=NextWord(cp);
      if (!ValidFloat(cp,NULL)) gnom_fatal(__FILE__,__LINE__,"Could not read y size");
      feed_si->InYSize = GetFloat(cp,NULL);
      sprintf(temp_err_string, "YSize = %6d", feed_si->InYSize); gnom_report(temp_err_string);
      cp=NextWord(cp);
      if (feed_s->mode==MODE_GNOMONIC)
       {
        ...
        sprintf(temp_err_string, "Central RA = %.6f hr", feed_si->InRA0/M_PI*12.); gnom_report(temp_err_string);
        cp=NextWord(cp);
        if (!ValidFloat(cp,NULL)) gnom_fatal(__FILE__,__LINE__,"Could not read central Dec");
        feed_si->InDec0 = GetFloat(cp,NULL) * M_PI/180.;
        sprintf(temp_err_string, "Central Dec = %.6f deg", feed_si->InDec0/M_PI*180.); gnom_report(temp_err_string);
        cp=NextWord(cp);
        ...
        sprintf(temp_err_string, "Rotation = %.6f deg", feed_si->InRotation/M_PI*180.); gnom_report(temp_err_string);
        cp=NextWord(cp);
        ...
        sprintf(temp_err_string, "XScale = %.6f deg/width", feed_si->InXScale/M_PI*180.); gnom_report(temp_err_string);
        cp=NextWord(cp);
        ...
        sprintf(temp_err_string, "YScale = %.6f deg/height", feed_si->InYScale/M_PI*180.); gnom_report(temp_err_string);
       }
      else
       {
        ...
        sprintf(temp_err_string, "X Shift = %.2f pixels", feed_si->InYOff); gnom_report(temp_err_string);
        cp=NextWord(cp);
        if (!ValidFloat(cp,NULL)) gnom_fatal(__FILE__,__LINE__,"Could not read y offset");
        feed_si->InYOff = GetFloat(cp,NULL);
        sprintf(temp_err_string, "Y Shift = %.2f pixels", feed_si->InXOff); gnom_report(temp_err_string);
        cp=NextWord(cp);
        if (!ValidFloat(cp,NULL)) gnom_fatal(__FILE__,__LINE__,"Could not read linear rotation");
        feed_si->InLinearRotation = GetFloat(cp,NULL) * M_PI/180.;
        sprintf(temp_err_string, "Rotation = %.6f deg", feed_si->InLinearRotation/M_PI*180.); gnom_report(temp_err_string);
       }
      (*nImages)++;
     }
