...
Usage: camfit.bin <filename>\n\
  ...
  for (i=1; i<argc; i++)
   {
    ...
    {
     sprintf(temp_err_string, "Received switch '%s' which was not recognised.\nType 'camfit.bin -help' for a list of available commandline options.", argv[i]);
  ...
  if (HaveFilename < 1)
   {
    sprintf(temp_err_string, "camfit.bin should be provided with a filename on the command line to act upon. Type 'camfit.bin -help' for a list of available commandline options.");
    gnom_error(ERR_GENERAL, temp_err_string);
    return 1;
   }
  else if (HaveFilename > 1)
   {
    sprintf(temp_err_string, "camfit.bin should be provided with only one filename on the command line to act upon. Multiple filenames appear to have been supplied. Type 'camfit.bin -help' for a list of available commandline options.");
    gnom_error(ERR_GENERAL, temp_err_string);
    return 1;
   }
...

  {
   ...
   gsl_vector_set(x,0,s_in_default.barrel_a); gsl_vector_set(ss,0,0.01);
   gsl_vector_set(x,1,s_in_default.barrel_b); gsl_vector_set(ss,1,0.01);
   gsl_vector_set(x,2,s_in_default.barrel_c); gsl_vector_set(ss,2,0.01);
